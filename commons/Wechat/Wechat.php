...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     * 获取已保存的access_token，从缓存或数据库获取
     * @author zithan
     */
    protected function getAccessTokenByDB($appid = '')
    {
        if (!$appid) {
            $appid = $this->appid;
        }

        $result = \WechatModel::getAccessToken($appid);
        //dump($result);
        if (!$result) {
            return false;
        }
     ...
     * @author zithan
     */
    protected function setAccessTokenByDB($accessToken)
    {
        return \WechatModel::setAccessToken($accessToken, $this->appid);
    }

    /**
     * @author zithan
     */
    /*protected function getJsTicketByDB($appid = '')
    {
        if (!$appid) {
            $appid = $this->appid;
        }
     ...
     * @author zithan
     */
    /*protected function setJsTicketByDB($accessToken)
    {
        //dump($accessToken);
        return \WechatModel::setAccessToken($accessToken, $this->appid);
    }*/
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		// @author zithan
		if ($accessToken = $this->getAccessTokenByDB($appid)) {
		...
		if ($result)
		{
            ...
            $this->setAccessTokenByDB($this->access_token);
			return $this->access_token;
		}
		return false;
	}
